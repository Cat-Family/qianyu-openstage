...
import { ResourceResponse } from '../../../ts/types/interface/menu.res.interface';
import classes from './Navbar.module.css';

const Navbar = () => {
  const [links, setLinks] = useState<any>();
  const { fetchData, data, loading } = useFetch<ResourceResponse>('', false);
  const [value] = useCookie('qy');

  useEffect(() => {
    value && fetchData('/resource/queryResourceTree', { method: 'POST' });
  }, []);
